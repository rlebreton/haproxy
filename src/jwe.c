...
static int decrypt_cek_aeskw(struct buffer *cek, struct buffer *decrypted_cek,
                             struct buffer *secret, jwe_alg crypt_alg)
{
	...
	if (!iv)
		goto end;
	/* Default IV for AES KW https://datatracker.ietf.org/doc/html/rfc3394#section-2.2.3.1 */
