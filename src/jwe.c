...
static int sample_conv_jwt_decrypt_secret(const struct arg *args, struct sample *smp, void *private)
{
	...
	if (args[0].type == ARGT_VAR) {
		...
		if (!secret)
			goto end;
		size = base64dec(secret_smp.data.u.str.area, secret_smp.data.u.str.data,
		                 secret->area, secret->size);
		if (size < 0)
			goto end;
		secret->data = size;
...
// 	size_t outl = b_size(decrypted_cek);
	...
	struct buffer *derived_key = NULL;
